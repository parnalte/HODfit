...
class HankelTransform(object):
    ...
    def __init__(self, nu=0, N=200, h=0.05):
        ...
        self.psi, self.dpsi = self._psi_dpsi(h * self._zeros)
        self.x = self._x(h)
    ...
    def _psi_dpsi(self, t):
        x = np.cosh(t)
        y = np.sinh(t)
        psi = t * np.tanh(np.pi * y / 2.)
        dpsi = (np.pi * t * x + np.sinh(np.pi * y)) / (1.0 + np.cosh(np.pi * y))
        dpsi[np.isnan(dpsi)] = 1.0
        return psi, dpsi
