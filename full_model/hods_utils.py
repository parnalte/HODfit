...
def pk2xir_hankel(rvalues, pkfunction, hankelN=6000, hankelh=0.0005):
    ...
    with nostdout():
        sph_hankel = hankel2.SphericalHankelTransform0(N=hankelN, h=hankelh)
...
def xir2wp_pi(rpvals, rvals, xivals, logpimin=-4, logpimax=3, npi=500):
    ...
    is defined by a set of tabulated values. We just create a linear interpolator
    of it and pass it to the function.
    """
    
    #Now, checks on tabulated values of xi(r)
    if not (sorted(rvals) == rvals).all():
        raise UserWarning("In function xir2wp_pi: the values of r you gave me were not sorted!")
    ...
    return xir2wp_xifunc_pi(rpvals=rpvals, xirfunc=xifunc, logpimin=logpimin, logpimax=logpimax, npi=npi)
