...
\title{Description of the HOD model computed by the ``HOD simple mode'' code}
\author{Pablo Arnalte-Mur (ICC-Durham)}
...
\begin{equation}
1 + \xi_{CS}(r) = \frac{2}{n_{gal}^2} \int_0^{+\infty} \mathrm{d}M n(M,z) \left[N_C(M) N_S(M)\right]\rho_h(r|M)
\end{equation}
